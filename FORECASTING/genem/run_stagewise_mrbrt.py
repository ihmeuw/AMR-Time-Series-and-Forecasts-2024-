...
def stagewise_mrbrt_all_omegas(
    ...
    cleaned_data_sub = _subset_ages_and_sexes(
        cleaned_data, gbd_round_id, age_standardize
    )
    ...
    prepped_input_data = prepped_input_data.sel(
        location_id=list(intersection_locations)
    )
    ...
    for omega in pv.get_omega_weights(omega_min, omega_max, step):
        ...
        if (
            ...
            forecast_data = _fit_and_predict_model(
                ...
                scenario_quantiles=(
                    SCENARIO_QUANTILES if use_scenario_quantiles else None
                ),
                gbd_round_id=gbd_round_id,
                stage=stage,
                entity=entity,
        ...
        if predictive_validity:
            ...
            for scenario in [-1, 1]:
                prepped_output_data.loc[{"scenario": scenario}] = (
                    prepped_output_data.sel(scenario=scenario).mean("draw")
                )
...
def _fit_and_predict_model(
    ...
    save_entity = (
        f"{entity}_sex_id_{sex_id}_omega_{omega}"
        if sex_id
        else f"{entity}_omega_{omega}"
    )
...
def _get_covariate_data(
    ...
    for cov_stage, cov_processor in covariates.items():
        ...
        cov_past_data = open_xr_scenario(cov_past_file_spec).sel(
        ...
        if DimensionConstants.STATISTIC in cov_past_data.dims:
            cov_past_data = cov_past_data.sel(
                statistic=DimensionConstants.MEAN, drop=True
            )
        if DimensionConstants.STATISTIC in cov_future_data.dims:
            cov_future_data = cov_future_data.sel(
                statistic=DimensionConstants.MEAN, drop=True
            )
...
def limit_scenario_quantiles(da: xr.DataArray) -> xr.DataArray:
    ...
    limited_da = xr.concat(
        [
            limited_worse,
            expand_dimensions(
                ref, scenario=[ScenarioConstants.REFERENCE_SCENARIO_COORD]
            ),
            limited_better,
        ],
        dim="scenario",
