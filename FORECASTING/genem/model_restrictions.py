...
class ModelRestrictions:
    ...
    def __init__(
        self, restrictions: Iterable[Tuple[str, Union[int, str], str]]
    ) -> None:
        """Initializer.
        ...
        for restriction in self._original_specification:
            ...
            location = (
                ALL_CATEGORIES
                if restriction[1] == ALL_CATEGORIES
                else int(restriction[1])
            )
    ...
    def string_specifications(self) -> List[str]:
        ...
        """
        return [
            " ".join([str(field) for field in spec])
            for spec in self._original_specification
        ]
    ...
    def yaml_representer(dumper: Any, data: ModelRestrictions) -> str:
        ...
        return dumper.represent_sequence(
            "tag:yaml.org,2002:seq", data._original_specification
        )


# register the yaml_representer with pyyaml's safedumper (which is what we use to write yaml)
yaml.SafeDumper.add_representer(ModelRestrictions, ModelRestrictions.yaml_representer)
