...
from amr_prep.utils.amr_io import AmrResult, get_amr_results
from cod_prep.utils import print_log_message, report_duplicates, report_if_merge_fail
from mcod_prep.compile_burden import summarize_draws, summarize_draws_log_transform
...
class AmrSummarizer(object):
    ...
    def __init__(self, burden_type, year_id, cause_id, infectious_syndrome,
        ...
        self.num_draws = self.conf.get_id('draw_num')
        self.id_cols = [
        ...
        self.draw_cols = ["draw_" + str(x) for x in range(0, self.num_draws)]
    ...
    def simple_aggregate(self, df, col_to_aggregate, detail_to_agg_map,
        ...
        value_cols = value_cols or self.draw_cols + ['point_estimate']
    ...
    def aggregate_location(self, df):
        ...
        logger = setup_logger(
            f"summarize_burden_{self.cause_id}_{self.infectious_syndrome}_{self.pathogen}"
        )
        return aggregate_locs(
            df, lhh, logger, draw_cols=self.draw_cols + ['point_estimate'], id_cols=self.id_cols,
    ...
    def calculate_dalys(self, df):
        ...
        dalys = df.query("measure_id == 3").append(ylls)\
            .assign(measure_id=2)\
            .groupby(self.id_cols, as_index=False)[self.draw_cols + ['point_estimate']].sum()
        return dalys
    ...
    def calculate_rates(self, df):
        ...
        df[self.draw_cols + ['point_estimate']] = pd.DataFrame(
            df[self.draw_cols + ['point_estimate']].to_numpy() / df[['population']].to_numpy(),
            index=df.index
        )
        ...
        age_std[self.draw_cols + ['point_estimate']] = pd.DataFrame(
            age_std[self.draw_cols + ['point_estimate']].to_numpy() * age_std[['age_group_weight_value']].to_numpy(),
            index=age_std.index
        )
        age_std = age_std.assign(age_group_id=27)
        age_std = age_std.groupby(self.id_cols)[self.draw_cols + ['point_estimate']].sum().reset_index()
    ...
    def summarize(self):
        ...
        df = df.groupby(self.id_cols, as_index=False)[self.draw_cols + ['point_estimate']].sum()
        ...
        df = df.append(self.aggregate_age(df, agg_age_group_ids=[42, 179, 192, 4, 5, 1, 239, 40, 23, 24, 25, 26, 22]))
        ...
        AmrResult(
            ...
            save_draws=True
        ).write_results(df.assign(metric_id=1))
        ...
        df_assc = df.loc[df['counterfactual'] == 'no_infection', ].reset_index(drop=True)
        df = df.loc[df['counterfactual'] == 'susceptible_infection', ].reset_index(drop=True)
        df_assc = summarize_draws_log_transform(df_assc, draw_cols=self.draw_cols, prefix='amr_')
        df = summarize_draws(df, draw_cols=self.draw_cols, prefix='amr_')
        df = pd.concat([df, df_assc]).reset_index(drop=True)
        if 'susceptible_infection' in df['counterfactual'].unique().tolist():
            df.loc[
