...
class AmrMultiplier(object):
    ...
    def __init__(self, process, burden_type, year_id=None, cause_id=None,
        ...
        self.cache_kwargs = {'force_rerun': False, 'block_rerun': True}
    ...
    def launch(self):
        ...
        process_resource_dict = {
            'split_sepsis_syndrome': {
                'num_workers': 10, 'memory': '10G', 'runtime': "04:30:00",
                'params': [10]
            },
            'calculate_mi_ratios': {
                'num_workers': 20, 'memory': '40G', 'runtime': "04:30:00",
                'params': [20]
            },
            'split_pathogen': {
                'num_workers': 1, 'memory': '20G', 'runtime': "02:00:00",
                'params': [self.burden_type]
            },
            'calculate_ylds_per_case': {
                'num_workers': 20, 'memory': '40G', 'runtime': "05:00:00",
                'params': [20]
            },
            'calculate_amr_props': {
                'num_workers': 1, 'memory': '10G', 'runtime': "02:00:00",
                'params': [self.burden_type]
            },
            'calculate_amr_burden': {
                'num_workers': 1, 'memory': '8G', 'runtime': "04:00:00",
                'params': [self.burden_type]
            },
            'aggregate_cause_syndrome': {
                'num_workers': 1, 'memory': '50G', 'runtime': "02:30:00",
                'params': [self.burden_type, 1]
            },
            'summarize_burden': {
                'num_workers': 1, 'memory': '185G', 'runtime': "03:30:00",
                'params': [self.burden_type, 1]
            }
        }
        if self.process != 'aggregate_cause_syndrome':
            ...
            self.tasks.to_csv("FILEPATH", index=False)
    ...
    def delete_outputs(self):
        for index, row in tqdm(self.expected_files.iterrows(), total=self.expected_files.shape[0]):
            AmrResult(
                self.process, self.burden_type, **row.to_dict()
            ).clear_results()
            if self.process == 'summarize_burden':
                AmrResult(
                    self.process, self.burden_type, **row.to_dict(), save_draws=True
                ).clear_results() 
    ...
    def check_last_run(self, save_path=None):
        ...
        if (df.exit_status != 0).any():
            if save_path is not None:
                df.loc[df.exit_status != 0].to_excel(
                    "FILEPATH",
                    index=False
                )
            print_log_message(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--year_id', nargs='*', type=int, default=list(range(1990, 2022)))
    ...
    parser.add_argument('--delete_outputs', action='store_true')
    ...
    delete_outputs = kwargs['delete_outputs']
    for kwarg in ['check_outputs', 'check_last_run', 'failed_jobs_save_path', 'delete_outputs']:
        kwargs.pop(kwarg)
