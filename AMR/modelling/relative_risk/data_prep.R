  ...
  df <- assign_l2_syndrome(df, available_parents, infsyn_hierarchy)
    df <- df %>%
      ...
      mutate(infectious_syndrome = case_when(
        infectious_syndrome %in% c("L2_OAMOIS", "L2_unspecified_site_infection") ~ "L2_unspecified",
        TRUE ~ infectious_syndrome
        )) %>%
      rename(infectious_syndrome_old = infectious_syndrome) %>%
      mutate(infectious_syndrome = case_when(
        infectious_syndrome_old != "L2_blood_stream_infection" & 
          infectious_syndrome_old != "L2_lower_respiratory_infection" & 
          infectious_syndrome_old != "L2_unspecified" &
          infectious_syndrome_old != "L2_urinary_tract_infection" ~ "other_syndromes",
        TRUE ~ infectious_syndrome_old
      ))
...
clean_time <- function(df){
  df$days_infection[df$days_infection < 0] <- 0L
  df$days_prior[df$days_prior < 0] <- 0L
  
  return(df)
}

check_for_los <- function(df){
    if('days_infection' %in% colnames(df)) { 
      check <- TRUE
    }else{
      check <- FALSE
    }
...
if (burden == "nonfatal"){
  check <- check_for_los(df)
  if (check == FALSE){
    stop(paste0(source, " does not have length of stay ; terminated"))
  }
}

if (source %in% c("cassini_rr_los", "taiwan")){
...
df <- df %>%
  dplyr::group_by(sample_id, abx_class, pathogen, infectious_syndrome, source) %>%
  dplyr::summarize(resistant = max(resistant),
                   cases = max(cases),
                   !!sym(val) := max(!!sym(val)), .groups = 'drop')
