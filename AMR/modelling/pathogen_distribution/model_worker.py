...
def main(model_version, infectious_syndrome, holdout, ref_pathogen):
    ...
    model_dir = PathogenNetwork.out_dir / infectious_syndrome / model_version
    df = pd.read_csv(model_dir / "input_data.csv")
    with open(model_dir / "dorm_models.pkl", 'rb') as file:
    ...
    out_file = model_dir / {'no_holdout': ''}.get(
